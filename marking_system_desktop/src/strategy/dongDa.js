export default (data, chekced) => {
  ...
  __keys.forEach(item => {
    rooms[item].roomAverageScore = parseInt(rooms[item].score / rooms[item].num)
    __allRoomsScore += rooms[item].roomAverageScore
  })
  // 得到所有房间的总平均分
  const __allRoomsScoreAve = parseInt(__allRoomsScore / __keys.length)
  // 计算东大策略下的分数
  __allScoreData = __allScoreData.map(item => {
    let dongDaScore = '暂无'
    if (item.averageScore !== '暂无') {
      dongDaScore = parseInt(item.averageScore / rooms[item.markId].roomAverageScore) * __allRoomsScoreAve
    }
